...
import { trainerMenuItems } from "@/lib/menu-data";
...
export default function MainLayout({
  ...
  useEffect(() => {
    ...
    if (user_get) {
      setUser({
        name: user_get.name || user_get.username || "Trainer",
        icon: null
      });
    }
  }, []);
  ...
  const findMenuLabel = (path: string): string => {
    const mainItem = trainerMenuItems.find(item => item.pathname === path);
    if (mainItem) return mainItem.label;

    for (const item of trainerMenuItems) {
      if (item.subMenu) {
